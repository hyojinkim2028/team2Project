...
document.addEventListener('DOMContentLoaded', function () {
  ...
  fetch(`https://api.themoviedb.org/3/movie/${paramId}/release_dates`, options)
    ...
    //비디오 api
  fetch(`https://api.themoviedb.org/3/movie/${paramId}/videos?language=en-US`, options)
    .then(response => response.json())
    .then(response => {
      document.getElementById("youtubeFrame").setAttribute('src',`https://www.youtube.com/embed/${response.results[0].key}`) 
      console.log(response.results[0].key)})// key값 불러오는 거
    .catch(err => console.error(err));
